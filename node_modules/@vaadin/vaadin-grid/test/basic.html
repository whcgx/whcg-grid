...
describe('basic features', () => {
  ...
  it('check visible item count', () => {
    grid.size = 10;
    flushGrid(grid);
    expect(dom(grid.root).querySelectorAll('tbody tr:not([hidden])').length).to.eql(10);
  });
  ...
  it('should have flex: 1 1 100% style', () => {
    ...
    expect(window.getComputedStyle(grid).getPropertyValue('flex-basis')).to.equal('100%');
  });
