...
function getVisualCellContent(section, row, col) {
  ...
  if (section.id === 'footer') {
    cell = dom(section).querySelector('[part~="cell"]');
  }
...
describe('reordering simple grid', () => {
  ...
  it('should not adjust outer-scroller z while reordering', () => {
    const zIndex = window.getComputedStyle(grid.$.outerscroller).zIndex;
    dragStart(headerContent[0]);
    grid._scrollHandler();
    expect(window.getComputedStyle(grid.$.outerscroller).zIndex).to.equal(zIndex);
  });
  ...
  describe('frozen columns', () => {
    ...
    beforeEach(() => {
      const columns = dom(grid).querySelectorAll('vaadin-grid-column');
